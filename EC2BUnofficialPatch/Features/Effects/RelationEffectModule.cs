    ...
    internal static class RelationEffectPatches
    {
            ...
            int ___mapId)
        {
            ...
            if (applied)
            {
                ...
                // 520 and 521 are silent recovery effects. Every negative leave
                // state reports its result even when TalkCfg passes _toast=false.
                if (subType < 0 && !string.IsNullOrEmpty(text))
