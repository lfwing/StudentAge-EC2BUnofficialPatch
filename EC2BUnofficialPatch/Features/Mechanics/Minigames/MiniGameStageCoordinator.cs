...
namespace EC2BUnofficialPatch.Features.Mechanics.Minigames
{
    internal sealed class MiniGameStageSession
    {
        ...
        internal bool Begun;
        internal CustomMinigameContext ExternalContext;
    ...
    internal static class MiniGameStageCoordinator
    {
        ...
        [ThreadStatic] private static long _adapterEndGameToken;
            ...
            out bool result)
        {
            ...
            {
                PatchLog.Warning("机制模块-已有小游戏阶段进行中，拒绝第二次启动");
                return true;
            }
            ...
            bool deferred = plan.Fallback?.Kind == CustomMinigameKind.External &&
                plan.Fallback.Definition.Parameters != null &&
                plan.Fallback.Definition.Parameters.TryGetValue("deferStart", out string defer) &&
                string.Equals(defer, "true", StringComparison.OrdinalIgnoreCase);
            if (!deferred) Singleton<RoleMgr>.Ins.GetRole().UpdateAttr(3, -cost, 1f, name, 2);
            ...
            {
                RefundFailedStart(state, deferred ? 0 : cost, name);
                PatchLog.Exception(
            ...
            MiniGameStageSession session = new MiniGameStageSession
            {
                ...
                Cost = deferred ? 0 : cost,
                PendingCost = deferred ? cost : 0,
        ...
        internal static bool ConsumeAdapterEndGame(long token)
        {
            if (token == 0 || _adapterEndGameToken != token) return false;
            _adapterEndGameToken = 0;
            return true;
        }
            ...
            string source)
        {
            ...
            try
            {
                ...
                _adapterEndGameToken = token;
                try { session.Owner.EndGame(session.LogicalGameId, isWin, selectId); }
                finally { _adapterEndGameToken = 0; }
        ...
        internal static void Tick()
        {
            ...
            try
            {
                // A loaded save owns another Role instance. Never refund against that new save.
                var state = Game.GetGameState();
                if (state != GameState.Start) s.SeenRunningWorld = true;
                if ((s.SeenRunningWorld && state == GameState.Start) || !ReferenceEquals(s.Player, Singleton<RoleMgr>.Ins.GetRole()))
                    Clear("world-changed");
            }
            catch { Clear("world-unavailable"); }
        }

        internal static bool BeginExternal(long token)
        {
            var s = Current;
            if (s == null || s.Token != token || s.Settled) return false;
            if (s.Begun) return true;
            if (s.PendingCost > 0)
            {
                if (!Singleton<RoleMgr>.Ins.HasEnoughCost(3, s.PendingCost, false)) return false;
                string name = DescCtrl.GetFromTag(Cfg.MinigameCfgMap[s.LogicalGameId].name);
                Singleton<RoleMgr>.Ins.GetRole().UpdateAttr(3, -s.PendingCost, 1f, name, 2);
                s.Cost += s.PendingCost; s.PendingCost = 0;
            }
            s.Begun = true; return true;
        }
        ...
        internal static void Clear(string reason)
        {
            ...
            Current = null;
            session?.ExternalContext?.Invalidate();
        }
