...
namespace EC2BUnofficialPatch.Features.Mechanics.Minigames
{
    internal static class MiniGameMechanicsPatches
    {
            ...
            ref EndGamePatchState __state)
        {
            ...
            bool adapterDispatch = MiniGameStageCoordinator.ConsumeAdapterEndGame(session.Token);
            if (session.Settled || (session.SettlementInProgress && !adapterDispatch))
            {
                return false;
            }
            ...
            if (!adapterDispatch && !session.EmbeddedLaunch &&
                activeOriginal != null &&
