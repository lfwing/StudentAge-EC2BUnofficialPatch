        ...
        private static void CheckForUpdates(CancellationToken token)
        {
            ...
            if (PluginConfig.UpdateAutoInstall?.Value != true)
            {
                ...
                PatchLog.Warning(
                    ...
                    $"自动安装已关闭，下载页={manifest.releasePage ?? manifestUrl}");
                return;
            }
            ...
            if (System.Reflection.AssemblyName.GetAssemblyName(pendingPath).Name != "EC2BUnofficialPatch")
                throw new InvalidDataException("更新程序集名称不是 EC2BUnofficialPatch");
            string helperPath;
            try
            {
        ...
        private static byte[] Download(string url, int maxBytes, int timeoutMs, CancellationToken token)
        {
            ...
            using (var response = (HttpWebResponse)request.GetResponse())
            {
                if (response.ResponseUri.Scheme != Uri.UriSchemeHttps)
                    throw new InvalidDataException("拒绝 HTTPS 降级重定向");
