...
namespace EC2BUnofficialPatch.Core
{
    internal static class TemplateMaintenance
    {
        ...
        private const string ExternalLive2DReadmeResource =
            "EC2BUnofficialPatch.Embedded.ExternalLive2D.Readme.txt";

        private const string ExternalLive2DJson =
            "{\r\n" +
            "  \"models\": []\r\n" +
            "}\r\n";
        ...
        internal static void Ensure()
        {
            ...
            WriteTemplate(
                Path.Combine(templateRoot, "ExternalLive2D"),
                ReadEmbeddedText(ExternalLive2DReadmeResource),
                "ExternalLive2D.json",
                ExternalLive2DJson);
        ...
        {
            using (Stream stream = typeof(TemplateMaintenance).Assembly
                .GetManifestResourceStream(resourceName))
            {
                if (stream == null)
                    throw new FileNotFoundException("找不到内嵌模板资源", resourceName);
